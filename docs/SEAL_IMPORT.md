...
Orca can import media history from a Seal backup JSON file:

```bash
orca import /path/to/seal-backup.json
```
...
Imported file paths are metadata only. Orca serves or deletes a path only when it
canonicalizes to a real file inside `ORCA_DOWNLOAD_DIR`; external paths and
...
large backups, allocate memory accordingly and back up the Orca data directory
before import.
