...
(function () {
  ...
  const DICT: Record<LangCode, Record<string, string>> = {
    en: {
      ...
      'permissionsPrompt.hint': 'Allow these so Orca can report downloads reliably in the background.',
      ...
      'settings.formatHint': 'The container downloads are merged into. MKV fits every codec and any number of subtitle tracks; MP4 is the most widely compatible.',
      'settings.formatLocked': 'Pinned by the ORCA_CONTAINER environment variable.',
      'settings.subs': 'Subtitles',
      'settings.subsHint': 'Downloads subtitles, embeds them in the video for offline playback, and keeps the original subtitle files alongside it.',
      ...
      'settings.archiveHint': 'Every entry Orca has ever recorded, one <code>extractor id</code> per line (e.g. <code>youtube dQw4w9WgXcQ</code>). Edit freely — add lines to mark items as already downloaded, or delete lines to let them re-download — then Save.',
      'settings.archiveSave': 'Save archive',

      'settings.logs': 'Error log',
      'settings.logsHint': 'The most recent errors Orca hit while reading or downloading links (newest first). Tap a row to expand the full message.',
    ...
    'zh-Hans': {
      ...
      'permissionsPrompt.hint': '授予以下权限，以便 Orca 在后台可靠地报告下载进度与结果。',
      ...
      'settings.formatHint': '下载合并后的封装格式。MKV 兼容所有编码和任意数量的字幕轨；MP4 的设备兼容性最广。',
      'settings.formatLocked': '已由 ORCA_CONTAINER 环境变量锁定。',
      'settings.subs': '字幕',
      'settings.subsHint': '下载字幕，内嵌到视频中以便离线播放，同时保留原始字幕文件。',
      ...
      'settings.archiveHint': 'Orca 记录过的每一条，每行一个 <code>extractor id</code>（例如 <code>youtube dQw4w9WgXcQ</code>）。可自由编辑——添加行以标记为已下载，删除行以允许重新下载——然后保存。',
      'settings.archiveSave': '保存存档',

      'settings.logs': '错误日志',
      'settings.logsHint': 'Orca 最近在解析或下载链接时遇到的错误（最新在前）。点按某行可展开完整信息。',
    ...
    'zh-Hant': {
      ...
      'permissionsPrompt.hint': '授予以下權限，讓 Orca 能在背景可靠地回報下載進度與結果。',
      ...
      'settings.formatHint': '下載合併後的封裝格式。MKV 相容所有編碼和任意數量的字幕軌；MP4 的裝置相容性最廣。',
      'settings.formatLocked': '已由 ORCA_CONTAINER 環境變數鎖定。',
      'settings.subs': '字幕',
      'settings.subsHint': '下載字幕，內嵌到影片中以便離線播放，同時保留原始字幕檔案。',
      ...
      'settings.archiveHint': 'Orca 記錄過的每一筆，每行一個 <code>extractor id</code>（例如 <code>youtube dQw4w9WgXcQ</code>）。可自由編輯——新增行以標記為已下載，刪除行以允許重新下載——然後儲存。',
      'settings.archiveSave': '儲存封存',

      'settings.logs': '錯誤日誌',
      'settings.logsHint': 'Orca 最近在解析或下載連結時遇到的錯誤（最新在前）。點按某列可展開完整訊息。',
